...
def fuzz_count(buf):
    # Check first if the buffer is actually a valid pdf file. If it can not be opened with pikepdf, then do not bother mutating...
    try:
        buf = buf[HEADER_SIZE:] # Cut out the header stuff...
        with pikepdf.open(io.BytesIO(buf)) as _:
            return _mutation_count
    except:
        return 0 # Do not try to mutate invalid inputs...
